class SearchDisplayWindow < ManagedDisplayWindow

  def initialize(name, title)
    super("search", name, title)
  end
  ...
  def display(results_formatter)
    ...
    # cache formatted id in buffer for possible lookup
    if defined?(results_formatter.formatted_id)
      formatted_id = results_formatter.formatted_id
      VIM::command("let b:rally_vars['formatted_id']='#{formatted_id}'")
    end
